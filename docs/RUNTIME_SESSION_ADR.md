...
Normative public semantics for local runtime authority sessions, ownership, caching, propagation, and failure behavior. This ADR is local-first and peer-oriented. Remote synchronization transport details are intentionally outside this public runtime document.
...
- [ADR: Local runtime session semantics](#adr-local-runtime-session-semantics)
  - [Purpose / scope guard](#purpose--scope-guard)
  - [Related documents](#related-documents)
  - [Same-host authority principle](#same-host-authority-principle)
  - [User-scoped daemon and session model](#user-scoped-daemon-and-session-model)
  - [Plumbing roles](#plumbing-roles)
  - [Authority session](#authority-session)
  - [Unlock persistence vs runtime session](#unlock-persistence-vs-runtime-session)
  - [Authority ownership and propagation](#authority-ownership-and-propagation)
  - [Session cache and residency](#session-cache-and-residency)
...
The purpose of this ADR is to constrain local implementation complexity and preserve Unix-oriented operational behavior. `seckitd` may remain public as peer-side/local runtime infrastructure when it stays transport-agnostic.

This public ADR does not define remote synchronization services, durable delivery queues, deployment runbooks, or operational telemetry.
...
- A user-scoped `seckitd` is local peer/runtime plumbing for that user on that host, not a centralized authority service or control plane.

## Plumbing roles

| Role           | Session-relevant responsibility                                                                                            |
| -------------- | -------------------------------------------------------------------------------------------------------------------------- |
| `seckitd`      | Same-user local peer/runtime mediator; may participate in authorized materialization; not source-of-truth vs `SecretStore` |
| `seckit`       | Operator commands, import/export, verification, and merge logic                                                            |
| `SecretStore`  | Authoritative local persistence and resolve primitives for secrets on this host                                            |
...
When a user-scoped daemon holds daemon-bound state, handles must invalidate fail-closed on daemon restart unless an explicit re-handshake path is documented.
...
- IPC consumers receive materialized plaintext as a crossing; they do not acquire `SecretStore` ownership or unlock lineage.
...
3. Injection carries materialized bytes, not unlock or `SecretStore` ownership.
...
This public document does not define remote synchronization services, remote APIs, distributed coordination, durable delivery queues, deployment orchestration, or operational telemetry.
