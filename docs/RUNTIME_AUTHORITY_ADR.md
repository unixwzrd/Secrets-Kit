...
- **`RuntimeAccessResult`** (if present in code) is **informational / documentary only**. It **must not** redefine `SecretStore`, imply IPC, caching, or lease semantics, or become a stable daemon API **in this phase**. Existing resolve APIs stay unchanged; store methods **must not** return `RuntimeAccessResult`.
