...
Normative vocabulary for **protected authority handling**, **materialization**, **injection**, and **export** surfaces in Secrets Kit. This document is **semantic, documentary, and invariant-oriented** only: no daemon, IPC transport, resident caches, or lease enforcement.

## Related documents

- [LOCAL_FIRST_DATASTORE_ARCHITECTURE.md](LOCAL_FIRST_DATASTORE_ARCHITECTURE.md) — current datastore, transaction, and projection architecture
- [METADATA_SCHEMAS.md](METADATA_SCHEMAS.md) — current operator metadata-schema behavior
...
- **`RuntimeAccessResult`** (if present in code) is **informational / documentary only**. It **must not** redefine `SecretStore`, imply IPC, caching or lease semantics, or become a stable daemon API. Existing resolve APIs stay unchanged; store methods **must not** return `RuntimeAccessResult`.
- **`RuntimeLease`** is a **placeholder** only (optional fields, no validation). This document does **not** define lease enforcement, revocation, runtime expiration, policy engines, audit pipelines, or stable lease APIs.
...
## Consumer Vocabulary Constraints

Daemons or APIs **must reuse** this vocabulary: **resolve**, **materialize**, **inject**, and **exported** meanings **must not** be redefined by new layers. Plumbing may extend transport or scheduling without redefining these terms.
...
Endpoints **must** remain **local-first** by default: **no implicit remote trust** for authority. **Lease**, **policy**, and **audit** semantics are not specified by this ADR.
