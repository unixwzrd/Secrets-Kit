...
- [Taxonomy registry (entry types, kinds, tags)](#taxonomy-registry-entry-types-kinds-tags)
  - [Canonical semantics](#canonical-semantics)
  - [Canonical names (UUID stability)](#canonical-names-uuid-stability)
  - [Authority](#authority)
  - [CLI — `seckit taxonomy`](#cli--seckit-taxonomy)
    - [`taxonomy list`](#taxonomy-list)
    - [`taxonomy show`](#taxonomy-show)
    - [`taxonomy export` / `install`](#taxonomy-export--install)
  - [Related: `seckit set` normalization](#related-seckit-set-normalization)
  - [Keychain vs SQLite](#keychain-vs-sqlite)
...
Interactive writes show the transform and accept `Y` / decline `n` / `force-raw`. Non-interactive: `--accept-normalized` or fix spelling.
...
| `vocabulary.entry_type/kind/tag.upsert` transactions | Mutation log for replay and synchronization |
...
Common flags on subcommands: `--backend`.
...
- `--force-raw-name` — keep literal spelling (forks UUIDs)

See [CLI_REFERENCE.md](CLI_REFERENCE.md) for the full command tree.

## Keychain vs SQLite

Keychain stores vocabulary in the system object JSON (and per-secret metadata strings). SQLite adds relational projections and vocabulary transactions on write; vocabulary transaction text fields must be strings.
