...
Backend flags (`--backend`, `--sqlite-dev-mode`, `--keychain`) on **`sync import`** match other secret commands.
...
- remote synchronization transport operations
...
Use the **same** pattern with **`--backend keychain`** and your Keychain path if you prefer; below uses developer-mode SQLite.

### Machine 1 (Alice, exporter)

```bash
export SECKIT_SQLITE_PATH="$HOME/.config/seckit/alice-seckit.sqlite"
...
seckit sync export --backend sqlite --sqlite-dev-mode \
...
```bash
export SECKIT_SQLITE_PATH="$HOME/.config/seckit/bob-seckit.sqlite"
seckit identity init
...
seckit sync import ~/Downloads/peer-bundle.json --signer alice \
  --backend sqlite --sqlite-dev-mode --dry-run
seckit sync import ~/Downloads/peer-bundle.json --signer alice \
  --backend sqlite --sqlite-dev-mode --yes
seckit get --backend sqlite --sqlite-dev-mode \
...
On each machine, use the Keychain backend via **`--backend keychain`** (default on macOS), drop `SECKIT_SQLITE_PATH`, and set **`--service` / `--account`** as you normally would. The **bundle file** is still moved manually; only the **secret store** backend changes.

## Automated tests and registry `home`

The unit tests under `tests/test_peer_sync_*.py` use distinct temp directories as logical “homes” for SQLite stores and **`apply_peer_sync_import(..., home=...)`** so metadata merge targets the right **`registry.json`** without changing the process **`HOME`**. For full **`seckit`** integration, run the suite on macOS with **`security`** available. A GUI login session helps when tests need an unlocked login keychain.

## See also

- [SECURITY_MODEL.md](SECURITY_MODEL.md) — overall posture  
- [EXAMPLES.md](EXAMPLES.md) — examples and helper scripts
