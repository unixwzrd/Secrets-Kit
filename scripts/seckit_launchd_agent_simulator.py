...
def main() -> int:
    if len(sys.argv) != 7:
        print(
            ...
            "<output-file> <keychain> <mode> <launchd-target> <env-name> <seckit-bin>",
            file=sys.stderr,
        )
        return 2

    output_file, keychain, mode, launchd_target, name, seckit_bin = sys.argv[1:7]
    payload = {
    ...
    pathlib.Path(output_file).write_text(json.dumps(payload, sort_keys=True) + "\n", encoding="utf-8")
    return 0
