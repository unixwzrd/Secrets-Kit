...
GITHUB_REPO="${SECKIT_GITHUB_REPO:-${GITHUB_REPOSITORY:-}}"
...
Environment:
  SECKIT_GITHUB_REPO   GitHub repo for --github-tag (defaults to CI GITHUB_REPOSITORY)
EOF
}
...
verify_github_release() {
  ...
  [[ "$GITHUB_REPO" =~ ^[A-Za-z0-9_.-]+/[A-Za-z0-9_.-]+$ ]] || die "set SECKIT_GITHUB_REPO or CI GITHUB_REPOSITORY for --github-tag"
