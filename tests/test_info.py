...
from secrets_kit.cli.commands.info import _effective_backend, build_info_dict
from secrets_kit.cli.operator_defaults import initial_operator_defaults


class InfoCommandTest(unittest.TestCase):
    def test_info_backend_default_matches_cli(self) -> None:
        for defaults, expected in (({}, "sqlite"), ({"backend": "keychain"}, "keychain")):
            with mock.patch("secrets_kit.cli.commands.info._load_defaults", return_value=defaults):
                self.assertEqual(_effective_backend(args=argparse.Namespace()), expected)
