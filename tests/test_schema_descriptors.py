...
CUSTOM_TEST_SCHEMA_ID = schema_id_for_name(name="custom.test")
CUSTOM_WIDGET_SCHEMA_ID = schema_id_for_name(name="custom.secret.widget")
...
class SchemaDescriptorTest(unittest.TestCase):
    def test_parse_rejects_non_string_field_type(self) -> None:
        with self.assertRaises(ValidationError):
            parse_descriptor(
                payload={
                    "schema_id": CUSTOM_TEST_SCHEMA_ID,
                    "schema_version": 1,
                    "entry_type": "secret",
                    "entry_kind": "generic",
    ...
    def test_parse_rejects_reserved_seckit_custom_field(self) -> None:
        with self.assertRaisesRegex(ValidationError, "reserved for Seckit internal metadata"):
            parse_descriptor(
                payload={
                    "schema_id": CUSTOM_TEST_SCHEMA_ID,
                    "schema_version": 1,
                    "entry_type": "secret",
                    "entry_kind": "generic",
                    "fields": {"seckit_entry_id": {"type": "string"}},
                }
            )

    def test_bundled_seeds_load(self) -> None:
        seeds = load_bundled_seed_descriptors()
        self.assertIn(GENERIC_SCHEMA_ID, seeds)
        self.assertIn(API_KEY_SCHEMA_ID, seeds)
    ...
    def test_merge_field_collision_fails_without_replace(self) -> None:
        ...
        base.schemas[GENERIC_SCHEMA_ID] = SchemaDescriptor(
            schema_id=GENERIC_SCHEMA_ID,
            schema_version=1,
            entry_type="secret",
            entry_kind="generic",
        ...
        seeds = {
            GENERIC_SCHEMA_ID: SchemaDescriptor(
                schema_id=GENERIC_SCHEMA_ID,
                schema_version=1,
                entry_type="secret",
                entry_kind="generic",
    ...
    def test_merge_add_only_field_union(self) -> None:
        ...
        fields = result.document.schemas[GENERIC_SCHEMA_ID].fields
        self.assertIn("provider", fields)
        self.assertIn("endpoint", fields)
        self.assertEqual(result.document.schemas[GENERIC_SCHEMA_ID].schema_version, 2)
    ...
    def test_install_seed_file_roundtrip(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            path.write_text(
                json.dumps(
                    {
                        "schema_id": CUSTOM_WIDGET_SCHEMA_ID,
                        "schema_version": 1,
                        "entry_type": "secret",
                        "entry_kind": "widget",
            ...
            self.assertEqual(loaded.schema_id, CUSTOM_WIDGET_SCHEMA_ID)
