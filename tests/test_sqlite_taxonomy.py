...
from secrets_kit.taxonomy.registry_store import load_taxonomy_registry

_UUID_RE = re.compile(
    r"^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$",
    re.IGNORECASE,
)
_ENTRY_TYPE_ID_RE = re.compile(r"^et:" + _UUID_RE.pattern.removeprefix("^"))
_ENTRY_KIND_ID_RE = re.compile(r"^ek:" + _UUID_RE.pattern.removeprefix("^"))
...
class SqliteTaxonomyTest(unittest.TestCase):
    def test_bootstrap_seeds_type_kind_with_uuid_ids(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            with mock.patch.dict(os.environ, {SQLITE_PATH_ENV: path}, clear=False):
                conn = provision_sqlite_datastore(path=Path(path), storage_mode="encrypted")
                ensure_sqlite_node_identity(conn=conn)
                conn.close()
                load_taxonomy_registry(backend="sqlite")
                ...
                taxonomy_secret_count = conn.execute(
            ...
            for row in types:
                self.assertRegex(row["entry_type_id"], _ENTRY_TYPE_ID_RE)
                self.assertEqual(row["entry_type_id"], entry_type_id_for_name(name=row["name"]))
            for row in kinds:
                self.assertRegex(row["entry_kind_id"], _ENTRY_KIND_ID_RE)
            self.assertIn("api_key", {row["name"] for row in kinds})
            self.assertEqual(taxonomy_secret_count, 0)
    ...
    def test_set_secret_syncs_tag_assignments_by_uuid(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            with mock.patch.dict(
                ...
                self.assertRegex(secret_row["entry_type_id"], _ENTRY_TYPE_ID_RE)
                self.assertRegex(secret_row["entry_kind_id"], _ENTRY_KIND_ID_RE)
                rows = conn.execute(
                    """
