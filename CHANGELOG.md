...
- **What changed:** `.pre-commit-config.yaml` runs **ruff check** (via `ruff-pre-commit`) and **basedpyright** on `src/` and `tests/` before unit tests. Maintainer dev deps live in `pyproject.toml` (`[project.optional-dependencies].dev`); `pip install -e ".[dev]"` then `pre-commit install`. Operator `curl|bash` / `--dev` runtime install stays `-e .` only (no lint tools in the uv runtime).
...
- **Docs:** Install examples now show `wget -qO- ... | bash` alternatives alongside `curl -fsSL ... | bash`.
