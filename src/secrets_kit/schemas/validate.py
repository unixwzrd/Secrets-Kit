...
def validate_custom(*, metadata: EntryMetadata, descriptor: SchemaDescriptor) -> EntryMetadata:
    ...
    for key, value in metadata.custom.items():
        if is_internal_custom_field(key):
            continue
    ...
    for key, spec in descriptor.fields.items():
        if key in merged_custom and is_custom_value_set(merged_custom[key]):
            continue
        if "default" in spec:
            merged_custom[key] = spec["default"]
            continue
        if spec.get("required"):
            raise ValidationError(
                f"custom field {key!r} is required for schema {descriptor.schema_id!r}"
            )
