...
from secrets_kit.internal_metadata import is_reserved_custom_field
...
def parse_descriptor(*, payload: Mapping[str, Any]) -> SchemaDescriptor:
    ...
    try:
        validate_identifier(value=schema_id, expected_type="schema", field="schema_id")
    ...
    for key, spec in raw_fields.items():
        ...
        if is_reserved_custom_field(field_name):
            raise ValidationError(
                f"field {field_name!r} is reserved for Seckit internal metadata"
            )
        if not isinstance(spec, dict):
            raise ValidationError(f"field {field_name!r} must be an object")
        spec_extra = set(spec.keys()) - {"type", "required", "default", "category"}
        if spec_extra:
            raise ValidationError(
        ...
        if "required" in spec:
            normalized["required"] = bool(spec["required"])
        if "default" in spec:
            normalized["default"] = "" if spec["default"] is None else str(spec["default"])
        if "category" in spec:
            normalized["category"] = str(spec["category"])
