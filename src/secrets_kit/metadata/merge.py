...
def merge_entry_metadata(
    *,
    ...
    if base is not None:
        ...
        merged.entry_id = base.entry_id
        merged.sync_origin_host = base.sync_origin_host
        if base.schema_id:
    ...
    if overlay is not None:
        # These optional scalar fields share the same nonblank override rule.
        for field in ("comment", "source_url", "source_label", "last_rotated_at", "expires_at"):
            value = getattr(overlay, field)
            if _is_set_scalar(value):
        ...
        if overlay.entry_id:
            merged.entry_id = overlay.entry_id
        if overlay.sync_origin_host:
            merged.sync_origin_host = overlay.sync_origin_host
