...
class SecurityCliStore(SecretStore):
    ...
    def raw_metadata(self, *, service: str, account: str, name: str) -> Dict[str, Any]:
        """Read metadata attributes without requesting the Keychain password."""
        svc = backend_service_name(service=service, name=name)
    ...
    def list(
        ...
        proc = run_security_capture(args=["dump-keychain", self._target()])
        entries: list[EntryMetadata] = []
        for raw in parse_dump_keychain_output(raw="\n".join((proc.stdout, proc.stderr))):
            raw_account = str(raw.get("account", ""))
            service_name = str(raw.get("service_name", ""))
            if account and raw_account != account:
                continue
            if ":" not in service_name:
                continue
            logical_service, logical_name = service_name.split(":", 1)
            if service and logical_service != service:
                continue
            parsed = parse_keychain_comment(comment=str(raw.get("comment", "")))
            if parsed is not None:
                if service and parsed.service != service:
                    continue
                if account and parsed.account != account:
                    continue
