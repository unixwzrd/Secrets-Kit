...
_SQLITE_SUPPORTED_COMMANDS: frozenset[str] = frozenset(
    {
        "set",
        "get",
        "list",
...
def _apply_defaults(*, args: argparse.Namespace) -> None:
    ...
    for field, key in (("type", "type"), ("kind", "kind"), ("tags", "tags"), ("tag", "tags")):
        if hasattr(args, field) and not getattr(args, field):
            setattr(args, field, defaults.get(key))
    for field, key in (("rotation_days", "default_rotation_days"), ("rotation_warn_days", "rotation_warn_days")):
        # Explicit zero is meaningful; only None accepts a rotation default.
        if hasattr(args, field) and getattr(args, field) is None and defaults.get(key):
            setattr(args, field, int(str(defaults[key])))
    ...
    if hasattr(args, "backend") and getattr(args, "backend", None):
        ...
        if (
            ...
            and getattr(args, "command", None) not in _SQLITE_SUPPORTED_COMMANDS
        ):
            raise ValidationError(
                "backend=sqlite currently supports set/get/list/delete/explain/export/import/run/doctor/schema/service/taxonomy/peer operations"
            )
