...
from secrets_kit.backends.sqlite.node_identity import load_sqlite_node_identity_summary
...
def _sqlite_status_dict() -> Dict[str, object]:
    """Build SQLite status (path and schema)."""
    spath = sqlite_path()
    ...
    if spath.exists():
        try:
            ...
            try:
                ...
                status["encryption"] = {
                    "enabled": storage_mode == SQLITE_STORAGE_MODE_ENCRYPTED,
                    "mode": storage_mode,
                    "warning": (
                        None
                        if storage_mode == SQLITE_STORAGE_MODE_ENCRYPTED
                        else msg("cli.info.warning.sqlite_plaintext_storage")
                    ),
                }
                ...
                status["node_identity"] = (
                    ...
                    if identity is None
                        ...
                        "signing_public_key_fingerprint": identity.signing_public_key_fingerprint,
                        "encryption_algorithm": identity.encryption_algorithm,
                        "encryption_public_key_fingerprint": (
                            identity.encryption_public_key_fingerprint
                        ),
                ...
                conn.close()
        except (sqlite3.Error, SQLiteBackendError, OSError, ValueError) as exc:
...
def _print_sqlite_warnings(*, sqlite: Dict[str, object]) -> None:
    ...
    if isinstance(encryption, dict) and encryption.get("enabled") is False:
        print(encryption.get("warning") or msg("cli.info.warning.encryption_disabled"), file=sys.stderr)
...
def _print_info_text(*, data: Dict[str, object]) -> None:
    ...
    if sqlite.get("included") is False:
        ...
        if "storage_mode" in sqlite:
            lines.append(f"  storage_mode: {sqlite.get('storage_mode')}")
        identity = sqlite.get("node_identity")
        if isinstance(identity, dict):
            lines.append("  node_identity:")
            lines.append(f"    node_id: {identity.get('node_id')}")
            lines.append(f"    signing_algorithm: {identity.get('signing_algorithm')}")
            lines.append(
                "    signing_public_key_fingerprint: "
                f"{identity.get('signing_public_key_fingerprint')}"
            )
            lines.append(f"    encryption_algorithm: {identity.get('encryption_algorithm')}")
            lines.append(
                "    encryption_public_key_fingerprint: "
                f"{identity.get('encryption_public_key_fingerprint')}"
            )
            lines.append(f"    created_at: {identity.get('created_at')}")
            lines.append(f"    state: {identity.get('state')}")
        elif "node_identity" in sqlite:
            lines.append("  node_identity: (none)")
