...
def _apply_candidates(
    *,
    ...
    for key in sorted(candidates):
        ...
        if not candidate.metadata.entry_id:
            candidate.metadata.entry_id = new_entry_id()
        if dry_run:
        ...
        if args is not None:
            ...
            resolved_tags = [
                resolve_name_for_cli(label="tag", raw=tag, args=args)
...
def cmd_import_env(*, args: argparse.Namespace) -> int:
    ...
    def load_candidates() -> Dict[str, ImportCandidate]:
        ...
        if not raw_names:
            return merged
        names = [item.strip() for item in str(raw_names).split(",") if item.strip()]
        if not names or len(names) != len(set(names)):
            raise ValueError("--names must contain unique variable names")
        requested = {validate_key_name(name=name) for name in names}
        filtered = {
            key: candidate
            for key, candidate in merged.items()
            if candidate.metadata.name in requested
        }
        observed = {candidate.metadata.name for candidate in filtered.values()}
        missing = sorted(requested - observed)
        if missing:
            raise ValueError("requested dotenv variable is unavailable")
        return filtered
