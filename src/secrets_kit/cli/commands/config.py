...
from secrets_kit.cli.defaults import _CONFIG_STORABLE_KEYS, _load_defaults, _validate_config_entry
...
def cmd_config_show(*, args: argparse.Namespace) -> int:
    """Print defaults.json or effective defaults (files + env)."""
    try:
...
def cmd_config_unset(*, args: argparse.Namespace) -> int:
    ...
    try:
        ...
        print(
            json.dumps(
                {"saved": True, "removed": key, "defaults_path": str(defaults_path())},
                indent=2,
                sort_keys=True,
            )
        )
...
__all__ = ["cmd_config_show", "cmd_config_set", "cmd_config_unset", "cmd_config_path"]
