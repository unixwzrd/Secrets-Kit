...
def build_metadata(*, args: argparse.Namespace, name: str, source: str) -> EntryMetadata:
    ...
    entry_type = resolve_name_for_cli(label="entry_type", raw=raw_type, args=args)
    entry_kind = resolve_name_for_cli(label="entry_kind", raw=raw_kind, args=args)
    ...
    for tag in raw_tags:
        resolved_tags.append(resolve_name_for_cli(label="tag", raw=tag, args=args))
    ...
    if not merged.entry_id:
        merged.entry_id = new_entry_id()
    return merged
