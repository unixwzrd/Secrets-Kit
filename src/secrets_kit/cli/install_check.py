...
_SECKIT_MCP_LAUNCHER_PATH = Path(
    os.environ.get("SECKIT_MCP_LAUNCHER_PATH", Path.home() / ".local" / "bin" / "seckit-mcp")
)
...
def _check_config_writable(*, issues: list[str]) -> bool:
    ...
    try:
        ...
        # Exclusive random creation must not overwrite a pre-existing file or
        # follow a planted link. The context removes only its own probe.
        with tempfile.TemporaryFile(dir=config_dir, prefix=".install_check_probe-"):
            pass
...
def _check_launcher_runtime(*, issues: list[str]) -> bool:
    ...
    if not _SECKIT_LAUNCHER_PATH.is_file():
        ...
        ok = False
    if not _SECKIT_MCP_LAUNCHER_PATH.is_file():
        issues.append(f"MCP launcher not found: {_SECKIT_MCP_LAUNCHER_PATH}")
        ok = False
    ...
    if not runtime_bin.is_file():
        ...
        ok = False
    runtime_mcp_bin = runtime_root / "bin" / "seckit-mcp"
    if not runtime_mcp_bin.is_file():
        issues.append(f"runtime seckit-mcp binary not found via {source}: {runtime_mcp_bin}")
        ok = False
    return ok
...
def _backend_hints() -> dict[str, Any]:
    ...
    if system == "darwin":
        ...
        hints["note"] = "Linux installs use SQLite backend by default"
    return hints
...
def run_install_check() -> dict[str, Any]:
    ...
    result: dict[str, Any] = {
        ...
        "mcp_launcher": str(_SECKIT_MCP_LAUNCHER_PATH),
