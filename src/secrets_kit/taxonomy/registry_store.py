...
from secrets_kit.backends.common import BACKEND_SQLITE, BackendError, normalize_backend
...
def _registry_metadata(*, payload_json: str) -> EntryMetadata:
    ...
    return EntryMetadata(
        ...
        schema_id=schema_id_for_name(name="builtin.secret.generic"),
        schema_version=1,
...
def load_taxonomy_registry(
    *,
    ...
    if normalized_backend == BACKEND_SQLITE:
        document = load_taxonomy_seed_document()
        if bootstrap_if_missing:
            _project_sqlite_registry(registry=document, home=home)
        return document
...
def save_taxonomy_registry(
    *,
    ...
    normalized_backend = normalize_backend(backend)
    if normalized_backend == BACKEND_SQLITE:
        _project_sqlite_registry(registry=registry, home=home)
        return
...
def _project_sqlite_registry(
    *, registry: TaxonomyRegistryDocument, home: Path | None = None
) -> None:
    """Project SQLite vocabulary relationally without creating a synthetic secret."""
    conn = open_sqlite_backend(home=home)
    try:
        project_registry_vocabulary(conn=conn, document=registry)
        conn.commit()
    finally:
        conn.close()
