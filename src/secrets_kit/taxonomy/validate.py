...
def require_known_entry_type(
    *,
    ...
    if registry.find_entry_type(name=name) is None:
        raise ValidationError(f"unknown entry_type {name!r}; install via seckit taxonomy install")
...
def require_known_entry_kind(
    *,
    ...
    if registry.find_entry_kind(name=name) is None:
        raise ValidationError(f"unknown entry_kind {name!r}; install via seckit taxonomy install")


def policy_mode_active(*, force_policy: bool = False) -> bool:
    """Return True when writes should reject unknown vocabulary."""
    return force_policy
