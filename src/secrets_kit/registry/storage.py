...
Persistence for registry.json (inventory projection) and defaults.json.

Does not resolve metadata from backends; see ``secrets_kit.registry.resolve``.
"""
...
def registry_dir(*, home: Optional[Path] = None) -> Path:
    """Return local metadata projection directory path."""
    base = home or Path.home()
    return base / ".config" / "seckit"


def registry_path(*, home: Optional[Path] = None) -> Path:
    """Return local metadata projection file path."""
...
def ensure_registry_storage(*, home: Optional[Path] = None) -> Path:
    """Create local metadata projection storage with secure permissions."""
...
def load_registry(*, home: Optional[Path] = None) -> Dict[str, EntryMetadata]:
    """Load local inventory/recovery metadata into a keyed mapping."""
...
def save_registry(*, entries: Dict[str, EntryMetadata], home: Optional[Path] = None) -> None:
    """Persist the local inventory/recovery metadata projection."""
    rpath = ensure_registry_storage(home=home)
    _check_secure_perms(path=rpath, max_mode=0o600)
    serialized: List[Dict] = [
        asdict(item)
        for item in sorted(entries.values(), key=lambda m: (m.service, m.account, m.name))
    ]
...
def upsert_metadata(*, metadata: EntryMetadata, home: Optional[Path] = None) -> None:
    """Insert or update one local inventory/recovery metadata record."""
